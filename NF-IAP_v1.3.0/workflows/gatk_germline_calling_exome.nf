include HaplotypeCaller from '../NextflowModules/GATK/4.1.3.0/HaplotypeCaller_exome.nf' params(optional:"${params.haplotypecaller.optional}", mem: "${params.haplotypecaller.mem}", genome_fasta : "${params.genome_fasta}")
...
include GenotypeGVCFs from '../NextflowModules/GATK/4.1.3.0/GenotypeGVCFs_exome.nf' params(mem: "${params.genotypegvcfs.mem}", genome_fasta: "${params.genome_fasta}", genome_dbsnp: "${params.genome_dbsnp}")
include SplitIntervals from '../NextflowModules/GATK/4.1.3.0/SplitIntervals_exome.nf' params(optional: "${params.splitintervals.optional}")
  ...
  main:
   ...
   // SplitIntervals( 'break', Channel.fromPath(params.genome_interval_list) )
    ...
    if (sample_gvcfs && sample_bams){
      ...
      HaplotypeCaller( sample_bams)

      // Scatter existing GVCFs over intervals
      //sample_gvcfs_channel = sample_gvcfs.combine(haplotypecaller.out
...
//        }
     ...
     // HaplotypeCaller( sample_bams.combine(SplitIntervals.out.flatten()))
      gvcf_per_interval = HaplotypeCaller.out
       ...
       //  [run_id, interval, gvcfs, idxs, interval_files[0]]
      // }
      // Run haplotype calling per sample
	HaplotypeCaller(sampe_bams)
    ...
    GenotypeGVCFs(HaplotypeCaller.out)
  emit:
    GenotypeGVCFs.out
    HaplotypeCaller.out
}
